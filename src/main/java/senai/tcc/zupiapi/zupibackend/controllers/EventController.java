...
public class    EventController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<List<Event>> findAll(@PathVariable Long id) {
        List<Event> events = eventService.findAll(id);

        return ResponseEntity.ok().body(events);
    }

    @PostMapping
    public ResponseEntity<Event> add(@RequestBody EventDTO event) {
